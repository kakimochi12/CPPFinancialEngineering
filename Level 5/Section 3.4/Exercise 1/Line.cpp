...
Line::Line() : point_one(Point(0.0, 0.0)), point_two(Point(0.0, 0.0)) // default constructor using colon syntax to initialize point at origin and second point at origin as well
{
    
} 

Line::Line(const Point &start, const Point &end) : point_one(start), point_two(end) // parameterized constructor using colon syntax to set starting and ending variables
{
    
}

Line::Line(const Line &other) : point_one(other.point_one), point_two(other.point_two) // Copy constructor using colon syntax to set point one and two to our copy values
{
