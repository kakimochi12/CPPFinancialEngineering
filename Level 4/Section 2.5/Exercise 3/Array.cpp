...
{
    if (this == &source) // check for slef asignment
    {
        return *this;
    }
...
// allows modification of elements, falls back to first index if out of bounds
Point& Array::operator [] (int index)
{
    if (index < 0 || index >= m_size)
    {
        return m_data[0]; // per assignment instructions
    }
    return m_data[index];
}
